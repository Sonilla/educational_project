...
string — произвольная строка Функция должна возвращать значение True, если строка string представляет собой
...
def is_valid(string):
    return all([4 <= len(string) <= 6, all(map(lambda x: str(x).isdigit(), string))])
