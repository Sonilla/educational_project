...
для позиционных аргументов: <значение аргумента> <тип аргумента> для именованных аргументов: <имя переменной>
<значение аргумента> <тип аргумента> Примечание 1. При выводе позиционные аргументы должны быть расположены в порядке
...
def print_given(*args, **kwargs):
    ...
    if len(args) > 0:
        for i in args:
            print(i, type(i))
    if len(kwargs) > 0:
        for k, v in sorted(kwargs.items(), key=lambda x: str(x[0])):
            print(f'{k} {v}', type(v))
